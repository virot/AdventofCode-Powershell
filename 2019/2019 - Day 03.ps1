...
Function Move-WireLocation{
  ...
  Process
  {
    [int[]]$coords = $CurrentLocation -split ','
    $distance = $Move -replace '[^0-9]'
    switch ($Move[0]) {
      "R" {([int]$coords[0]+1)..([int]$coords[0] + $distance)|%{"$_,$($coords[1])"}}
      "L" {([int]$coords[0]-1)..([int]$coords[0] - $distance)|%{"$_,$($coords[1])"}}
      "U" {([int]$coords[1]+1)..([int]$coords[1] + $distance)|%{"$($coords[0]),$_"}}
      "D" {([int]$coords[1]-1)..([int]$coords[1] - $distance)|%{"$($coords[0]),$_"}}
    }
#    return $coords -join ','
  }
}
...
Function Find-ClosestCrossing{
  param([string[]]$Line1, [string[]]$Line2)
  Process{
    $Path1 = [System.Collections.ArrayList]@()
    $Path1.Add('0,0')|Out-Null
    $Path2 = [System.Collections.ArrayList]@()
    $Path2.Add('0,0')|Out-Null
    $possible = @()
    
    $CurrLoc = '0,0'
    ForEach ($entry in $Line1)
    {
      #Write-Host "$Entry"
      Move-WireLocation -CurrentLocation $CurrLoc $entry|ForEach {$CurrLoc = $_; $Path1.Add($_)|Out-Null}
    }

    $CurrLoc = '0,0'
    ForEach ($entry in $Line2)
    {
      #Write-Host "L2 $Entry"
      Move-WireLocation -CurrentLocation $CurrLoc $entry|ForEach {$Path2.Add($_)|Out-Null;$CurrLoc = $_;if ($Path1.contains($_)){$possible += "$_"}}
    }
    $lowest = 10000
    $possible|%{if(($_ -split ","|%{[math]::Abs($_)}|measure-object -sum).sum -lt $lowest){$lowest_sol =$_; $lowest = ($_ -split ","|%{[math]::Abs($_)}|measure-object -sum).sum}}
    Write-Host "Closest Manhattan: $lowest"
    $lowest = 300000
    $Possible |% {($Path1.indexOf($_) + $path2.indexOf($_))|?{$_ -lt $lowest}|%{$lowest = $_}}
    Write-Host ""
  }
}

measure-command{Find-ClosestCrossing -Line1 $line1 -Line2 $line2}

measure-command{Find-ClosestCrossing -Line1 @('R8','U5','L5','D3') -Line2 @('U7','R6','D4','L4')}